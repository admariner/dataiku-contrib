...
def get_params(config, client, project):
    ...
    assert (
        model_folder_id in list_folders
    ), f"Folder ID {model_folder_id} must be the id of a managed folder containing a model trained with the deeplearning-image plugin. The folder must belong to the project in which is executed the macro"
    ...
    if create_new_service:
        ...
        assert (
            service_id not in list_service
        ), f"Service ID {service_id} already in use, find a new id or uncheck the create new service option to use an existing service"


    else:
        service_id = config.get("service_id_existing")
        assert service_id, "Service ID is empty"
        assert service_id in list_service, f"Service ID : {service_id} not found"
        ...
        if endpoint_id in list_endpoints:
            print(f"Will override endpoint {endpoint_id}")
        else:
            print(f"Create new endpoint {endpoint_id} in service {service_id}")
...
def get_api_service(params, project):
    ...
    return (
        project.create_api_service(params.get("service_id"))
        if params.get('create_new_service')
        else project.get_api_service(params.get("service_id"))
    )
...
def get_model_endpoint_settings(params):
    ...
    folder_list = [{"ref": params.get("model_folder_id")}]
    ...
    test_query = {"name": "Score car image", "q": input_query}
    list_queries = [test_query]
    endpoint_settings["testQueries"] = list_queries

    code_template = """import dataiku
import pandas as pd
